...
# Attach S3-derived file sizes to the parsing status data
parsing_status_dec_fil <- parsing_status_dec |>
  filter(!is_skipped, is_parsed, is_in_trino, schema == "in-network-rates") |>
  left_join(
    file_sizes_dec |>
      rename(s3_total_size_bytes = total_size),
    by = c("payer_id", "data_source_name")
  )
parsing_status_feb_fil <- parsing_status_feb |>
  filter(!is_skipped, is_parsed, is_in_trino, schema == "in-network-rates") |>
  left_join(
    file_sizes_feb |>
      rename(s3_total_size_bytes = total_size),
...
network_names_per_file_feb <- payer_files_feb |>
  ...
  summarize(
    network_name_lst_feb = list(unique(str_trim(flatten_chr(network_names)))),
    network_name_n_uniq_feb = map_int(network_name_lst_feb, \(x) length(x))
  )
...
payer_df <- payer_files_dec |>
  ...
  left_join(
    parsing_status_dec_fil |>
      group_by(payer_id) |>
      summarize(
        num_files_dec = n(),
        tot_file_bytes_dec = sum(raw_file_size_bytes, na.rm = TRUE),
        avg_file_bytes_dec = mean(raw_file_size_bytes, na.rm = TRUE),
        tot_file_bytes_s3_dec = sum(s3_total_size_bytes, na.rm = TRUE),
        avg_file_bytes_s3_dec = mean(s3_total_size_bytes, na.rm = TRUE)
      ),
    by = "payer_id"
  ) |>
  left_join(
    parsing_status_feb_fil |>
      ...
      summarize(
        ...
        tot_file_bytes_s3_feb = sum(s3_total_size_bytes, na.rm = TRUE),
        avg_file_bytes_s3_feb = mean(s3_total_size_bytes, na.rm = TRUE)
      ),
    by = "payer_id"
  ) |>
  mutate(
    diff_files = num_files_feb - num_files_dec,
  ...
  left_join(
    network_names_per_file_feb |>
      group_by(payer_id) |>
      summarize(
        pct_file_w_network_name = mean(network_name_n_uniq_feb > 0, na.rm = TRUE),
        uniq_network_name_lst = list(unique(flatten_chr(network_name_lst_feb))),
        uniq_network_name_cnt = map_int(uniq_network_name_lst, length)
      ),
    by = c("payer_id")
  ) |>
  left_join(
    provider_mode_dec |>
      ...
      summarize(
        ...
        num_dsn_pg_reference_dec = sum(provider_mode == "reference")
      ),
    by = "payer_id"
  )
```
