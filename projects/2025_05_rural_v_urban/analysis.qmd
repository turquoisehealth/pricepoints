...
rates_clean_df |>
  ...
  labs(
    ...
    subtitle = paste0(
      "In some counties, the median reimbursement is over 350% of adjusted ",
      "Medicare costs, while<br>others have a median reimbursement of just ",
      "150% of Medicare costs (across all procedures)"
    ),
    caption = paste0(
      "Based on ",
      rates_clean_df |>
      ...
      "ratio of all providers, payers, and rates in the county\n",
...
rates_clean_df |>
  ...
  labs(
    ...
    subtitle = paste0(
      "Across a sample of 93M+ rates from over 5,000 hospitals, rural ",
      "hospitals have<br>more variable rates and are reimbursed",
...
nchs_agg_df <- rates_clean_df |>
  ...
  mutate(
    code_source = "NCHS Urban-Rural Classification Scheme",
    code_label = factor(nchs_code),
...
nchs_agg_df |>
  ...
  labs(
    ...
    subtitle = paste0(
      "Hospitals in noncore rural counties are reimbursed 33 percentage points",
...
nchs_class_agg_df <- rates_clean_df |>
  filter(!(is.na(all_wtd_mean_pct_of_medicare) | is.na(total_beds))) |>
  filter(!is.na(nchs_code)) |>
  group_by(nchs_class) |>
  summarize(
  ...
  mutate(
    ...
    class_label = fct_rev(class_label),
    code_class = nchs_class
  )
...
bind_rows(nchs_class_agg_df, rucc_agg_df, uic_agg_df, ruca_agg_df) |>
  mutate(
    code_source = factor(
      code_source,
      levels = c(
  ...
  guides(fill = "none") +
  facet_wrap(vars(code_source), ncol = 2, scales = "free_x") +
  labs(
    title = paste0(
      "The urban-rural reimbursement gap is consistent across\n",
      "different county-level rurality classification schemes"
    ),
    subtitle = paste0(
      "Most schemes (those with an asterisk) share the same urban/rural cutoff",
      " (from OMB 2023)"
    ),
    x = "",
    y = "Commercial to Medicare ratio",
    caption = paste0(
      "For NCHS, rural is codes ≥ 5. For RUCC/RUCA, rural is codes ≥ 4. ",
      "For Urban-Influence, rural is all codes not equal to 1 or 4\n",
      "Rates are sourced from Turquoise Health Clear Rates, ",
      "weighted by utilization, payer market share, and ",
      "provider bed count\n",
      "Box plots show weighted quantiles of commercial to Medicare rates ",
...
```{r classifications_breakout_boxplot, message=FALSE}
#| column: page
#| out-width: 100%
#| fig-width: 8
#| fig-height: 10

rucc_agg_br_df <- rates_clean_df |>
  filter(!(is.na(all_wtd_mean_pct_of_medicare) | is.na(total_beds))) |>
  filter(!is.na(rucc_code)) |>
...
uic_agg_br_df <- rates_clean_df |>
  ...
  mutate(
    code_source = "USDA Urban-Influence Codes*",
    code_label = factor(
      as.character(uic_code),
      levels = c("1", "4", "2", "5", "7", "3", "6", "8", "9")
    ),
    code_label = fct_relabel(
      code_label,
      \(x) {
...
bind_rows(nchs_agg_df, rucc_agg_br_df, uic_agg_br_df, ruca_agg_br_df) |>
  mutate(
    code_source = factor(
      code_source,
      levels = c(
  ...
  labs(
    title = paste0(
      "Urban-rural reimbursement patterns are relatively consistent"
    ),
    subtitle = paste0(
      "Across rurality classification schemes, ",
      "more rural almost always means lower reimbursement"
    ),
    x = paste0(
    ...
    y = "Commercial to Medicare ratio",
    caption = paste0(
      "* Urban-Influence Codes are re-ordered in terms of their rurality, ",
      "rather than their adjacency to populated areas\n",
