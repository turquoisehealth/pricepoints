...
nchs_agg_df |>
  ...
  labs(
    ...
    subtitle = paste0(
      "Hospitals in noncore rural counties are reimbursed 20 percentage points",
...
rucc_agg_df <- rates_clean_df |>
  ...
  mutate(
    code_source = "USDS Rural-Urban Continuum Codes",
...
uic_agg_df <- rates_clean_df |>
  ...
  mutate(
    code_source = "USDS Urban-Influence Codes",
...
ruca_agg_df <- rates_clean_df |>
  ...
  mutate(
    code_source = "USDS Rural-Urban Commuting Areas",
  ...
  mutate(
    code_source = factor(
      ...
      levels = c(
        ...
        "USDS Rural-Urban Continuum Codes",
        "USDS Rural-Urban Commuting Areas",
        "USDS Urban-Influence Codes"
      )
    )
  ) |>
  ggplot() +
...
rates_code_clean_df |>
  ...
  summarize(
    q10 = wtd.quantile(wtd_mean_pct_of_medicare, num_rates, 0.10, na.rm = TRUE),
    q25 = wtd.quantile(wtd_mean_pct_of_medicare, num_rates, 0.25, na.rm = TRUE),
    q50 = wtd.quantile(wtd_mean_pct_of_medicare, num_rates, 0.50, na.rm = TRUE),
    q75 = wtd.quantile(wtd_mean_pct_of_medicare, num_rates, 0.75, na.rm = TRUE),
    q90 = wtd.quantile(wtd_mean_pct_of_medicare, num_rates, 0.90, na.rm = TRUE)
  ) |>
  ungroup() |>
  mutate(q50_all = q50[nchs_class == "urban"], .by = service_line) |>
  mutate(service_line = fct_reorder(service_line, q50_all)) |>
  ggplot() +
  geom_boxplot(
    aes(
      xmin = q10,
