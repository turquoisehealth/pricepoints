...
bswh_inst_tq_cash_agg_df <- bswh_inst_tq_rates_df |>
  ...
  ungroup() |>
  mutate(total_beds_percentile = percent_rank(total_beds)) |>
  summarise(
    ymin = wtd.quantile(
      cash_pct_of_medicare,
      weights = total_beds_percentile * state_claims_percentile_all,
      probs = 0.10,
      na.rm = TRUE
    ),
    ylower = wtd.quantile(
...
bind_rows(bswh_inst_tq_payer_agg_df, bswh_inst_cpw_payer_agg_df) |>
  ...
  scale_y_continuous(labels = label_percent()) +
  scale_x_discrete(expand = expansion(mult = c(0.08, 0.0))) +
  labs(
    x = "",
    y = "Commercial to Medicare ratio",
    title = paste0(
      "Cost Plus Wellness has higher rates than large Texas insurers"
    ),
    subtitle = paste0(
      "Across Baylor Scott & White (BSW) hospitals and medical centers ",
      "in Dallas-Fort Worth"
...
bswh_inst_tq_payer_ipop_agg_df <- bswh_inst_tq_rates_df |>
  ...
  summarise(
    ymin = wtd.quantile(
      canonical_rate / medicare_rate,
      weights = total_beds_percentile * state_claims_percentile_sep,
      probs = 0.10,
      na.rm = TRUE
    ),
    ylower = wtd.quantile(
...
bind_rows(bswh_inst_tq_payer_ipop_agg_df, bswh_inst_cpw_payer_ipop_agg_df) |>
  mutate(
    ...
    payer_name = factor(
      payer_name,
      levels = c("UHC", "Cigna", "BSW Health", "Aetna", "BCBSTX", "Cost Plus")
    ),
    ymax = pmin(ymax, 5.5)
  ) |>
  arrange(payer_name) |>
  mutate(
    ...
    provider_label = paste0(
      ...
      "<span style='font-size:13px'> providers</span>"
    ),
    .by = payer_name
  ) |>
  mutate(
...
bind_rows(bswh_pro_tq_payer_agg_df, bswh_pro_cpw_payer_agg_df) |>
  ...
  labs(
    ...
    title = paste0(
      "Cost Plus ",
      "professional rates are comparable to large insurers'"
    ),
    subtitle = paste0(
