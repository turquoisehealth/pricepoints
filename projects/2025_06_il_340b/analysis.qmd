...
# Load OBBB KFF data
obbb_df <- read_parquet("data/output/obbb.parquet")
...
ggplot() +
  ...
  scale_y_continuous(
    name = "Hospital cash flows per 200mg dose of Keytruda",
...
```{r obbb_histogram, message=FALSE}
#| column: page
#| out-width: 100%
#| fig-width: 9
#| fig-height: 7
...
obbb_stats_df <- obbb_df |> 
  mutate(
    met_old = case_when(
      ...
      `340b_entity_type` %in% c("DSH", "PED") & mcr_dsh_pct > 0.1175 ~ TRUE,
      `340b_entity_type` == "CAH" ~ TRUE,
      .default = FALSE
    ),
    met_kff = case_when(
      `340b_entity_type` %in% c("SCH", "RRC") & mcr_dsh_pct_kff >= 0.08 ~ TRUE,
      `340b_entity_type` %in% c("DSH", "PED") & mcr_dsh_pct_kff > 0.1175 ~ TRUE,
      `340b_entity_type` == "CAH" ~ TRUE,
      .default = FALSE
    )
  )

obbb_thresholds_df <- tibble(
  type = rep("Before OBBB Medicaid cuts (2023)", 2),
  threshold = c(0.1175, 0.08),
  text = c(
    "**DSH<br>threshold**<br>(11.75%)",
    "**RRC/SCH<br>threshold**<br>(8.00%)"
  ...
  mutate(
    type = factor(
      type,
      levels = c(
        "Before OBBB Medicaid cuts (2023)",
        "After OBBB Medicaid cuts (2034)"
      )
    )
  )

obbb_note_df <- tibble(
  type = rep("After OBBB Medicaid cuts (2034)", 3),
  note = c(
    paste0(
      "<strong><span style='color:indianred'>Dark red</span></strong>",
      " hospitals ",
      "fall<br>below their threshold and<br>no longer qualify for 340B"
    ),
    paste0("<span style='font-size:40px'>←</span>"),
    paste0("Medicaid cuts shift the entire<br>distribution to the left")
  )
) |>
  mutate(
    type = factor(
...
obbb_stats_df |>
  filter(met_old, !is.na(mcr_dsh_pct)) |>
  select(`340b_entity_type`, mcr_dsh_pct, met = met_old) |>
  mutate(type = "Before OBBB Medicaid cuts (2023)") |>
  bind_rows(
    obbb_stats_df |>
      filter(met_old, !is.na(mcr_dsh_pct_kff)) |>
      select(
        `340b_entity_type`,
        mcr_dsh_pct = mcr_dsh_pct_kff,
        met = met_kff
      ) |>
      mutate(type = "After OBBB Medicaid cuts (2034)")
  ) |>
  filter(`340b_entity_type` != "CAH") |>
  mutate(
    type = factor(
      type,
      levels = c(
...
ggplot() +
  ...
  labs(
    title = "The OBBB may disqualify hundreds of 340B hospitals",
    subtitle = paste0(
      "Medicaid cuts could drop many hospitals below the 340B DSH ",
    ...
    caption = paste0(
      ...
      "2023 CMS/HCRIS reports ",
      "(Hospital 2552-10 form, Worksheet E, Part A, Line 33)\n",
      "Post-OBBB DSH percentage is estimated using data from the 2023 CMS ",
      "reports and state Medicaid cut estimates from the KFF"
    )
  ) +
  coord_cartesian(xlim = c(0.03, 0.25)) +
...
obbb_stats_df |>
  filter(met_old) |>
  group_by(mcr_state, met_kff) |>
  summarize(
    n = n(),
    count_state = first(`340b_hospital_count_state`),
    .groups = "drop"
  ) |>
  pivot_wider(names_from = met_kff, values_from = n) |>
  rename(
    "unchanged" = `TRUE`,
    "lost_340b" = `FALSE`
  ) |>
  mutate(
    lost_340b = replace_na(lost_340b, 0),
    pct_lost = lost_340b / count_state
  ) |>
  left_join(state_geo_df, by = c("mcr_state" = "abbr")) |>
...
ggplot() +
  ...
  scale_fill_brewer(
    palette = "BuPu",
    name = "Percent of existing 340B\nhospitals disqualified by\nOBBB Medicaid cuts"
  ) +
  labs(
    title = "The OBBB's impact on 340B-qualified hospitals, by state",
    subtitle = paste0(
      "States with large OBBB Medicaid cuts and many ",
      "marginally qualified 340B hospitals<br>",
      "(just above the DSH threshold) see the biggest losses in eligibility"
    ),
    caption = paste0(
      "Post-OBBB DSH percentage is estimated using data from the 2023 CMS ",
      "reports and state Medicaid cut estimates from the KFF\n",
      "Initial 340B status is from 2023 (to match CMS data). Sourced from the ",
      "HRSA OPAIS database, as of 2025-06-13\n"
    )
  ) +
  coord_sf(expand = FALSE, clip = "off") +
  guides(
    fill = guide_legend(override.aes = list(color = "grey20"), nrow = 5),
    color = "none"
  ) +
  theme_void() +
  theme(
    ...
    legend.margin = margin(t = 80, l = -30),
    legend.key.size = unit(0.75, "cm"),
    legend.text = element_text(size = 12),
    plot.title = element_text(
      size = 22,
      face = "bold",
      margin = margin(b = 6, l = 20, t = 10)
    ),
    plot.subtitle = element_markdown(
      size = 14,
      margin = margin(b = 16, l = 20, t = 2),
      lineheight = 1.3
    ),
    plot.caption = element_text(
      size = 11,
      lineheight = 1.1,
      margin = margin(r = -160, t = 20)
    )
  )
```

```{r obbb_table, message=FALSE}
#| column: page
#| out-width: 100%
#| fig-width: 9
#| fig-height: 7

obbb_type_vec <- c("DSH\n(Urban)", "DSH\n(Rural)", "RRC", "SCH", "CAH", "PED")
obbb_bed_vec <- c("0-50", "51-100", "101-200", "201-500", ">500")
obbb_stats_grid <- expand.grid(
  full_type = obbb_type_vec,
  bed_fct = obbb_bed_vec
)
...
obbb_stats_df |>
  ...
  mutate(
    ...
    full_type = case_when(
      `340b_entity_type` == "DSH" & mcr_urban_rural == 1 ~ "DSH\n(Urban)",
      `340b_entity_type` == "DSH" & mcr_urban_rural == 2 ~ "DSH\n(Rural)",
      `340b_entity_type` == "RRC" ~ "RRC",
      `340b_entity_type` == "SCH" ~ "SCH",
      TRUE ~ `340b_entity_type`
    ),
    full_type = factor(
      full_type,
      levels = obbb_type_vec
    ),
    lost_340b = !met_kff
  ) |>
  group_by(bed_fct, full_type) |>
  summarize(cnt = sum(lost_340b)) |>
  mutate(
    cnt_fct = cut(
      cnt,
  ...
  right_join(obbb_stats_grid, by = c("bed_fct", "full_type")) |>
  mutate(
...
ggplot() +
  ...
  geom_text(
    aes(x = bed_fct, y = fct_rev(full_type), label = cnt, color = cnt_fct),
    size = 5.5,
    show.legend = FALSE
  ) +
  scale_x_discrete(
  ...
  scale_y_discrete(
    name = "340B entity type (2023)",
    expand = expansion(mult = c(0, 0.05)),
    drop = FALSE
  ) +
  scale_fill_manual(
    values = c("white", RColorBrewer::brewer.pal(5, "Reds")),
    name = "Count of 340B hospitals\ndisqualified by OBBB\nMedicaid cuts",
    na.value = "grey80"
  ) +
  scale_color_manual(
    values = c(
  ...
  labs(
    ...
    subtitle = paste0(
      "Mid-sized DSH hospitals are the most likely to lose 340B eligibility"
    ),
    caption = paste0(
      "Table shows the number of hospitals that would lose 340B status ",
      "under the OBBB by 2034<br>",
      "Hospitals are grouped by 340B entity type, total beds, and their ",
      "urban/rural status from their 2023 CMS cost report<br>",
      "340B status is from 2023 (to match CMS data). Sourced from the ",
      "HRSA OPAIS database, as of 2025-06-13<br>",
      "Loss of 340B status is estimated using data from ",
      "2023 CMS cost reports and state Medicaid cut estimates from the KFF"
    )
  ) +
  theme_minimal() +
  theme(
...
```{r obbb_export}
obbb_stats_df |>
  filter(met_old, !met_kff) |>
  select(
    State = mcr_state,
    Provider = provider_name,
    CCN = mcr_ccn,
    `340B entity type (2023)` = `340b_entity_type`,
    `DSH % (2023 original)` = mcr_dsh_pct,
    `DSH % (2034 OBBB)` = mcr_dsh_pct_kff
  ) |>
  arrange(State) |>
  readr::write_csv("data/output/obbb_disqualified_340b_hospitals.csv")
```
