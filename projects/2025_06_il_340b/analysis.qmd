...
price_example_df |>
  mutate(across(where(is.numeric), \(x) round(x, 2) * 200)) |>
  pivot_longer(
    cols = avg_comm_rate:wac,
    names_to = "rate_type",
    values_to = "rate_amount"
  ) |>
  filter(rate_type %in% c(
    "asp", "gross_rate", "avg_comm_rate"
  )) |>
  mutate(
    rate_type = factor(
      rate_type,
    ...
    rate_type = fct_recode(
      rate_type,
      "Drug cost" = "asp",
      "Hospital list price" = "gross_rate",
      "Reimbursement price" = "avg_comm_rate"
    )
  ) |>
ggplot() +
  geom_bar(
    aes(x = rate_type, y = rate_amount, fill = rate_type),
    stat = "identity",
    width = 0.5
  ) +
  annotate(
  ...
  annotate(
    "segment",
    x = 1.25, xend = 1.25,
    y = 10000, yend = 31275 + 750,
    color = "grey20"
  ) +
  annotate(
    "segment",
    x = 1.25, xend = 1.82,
    y = 31275 + 750, yend = 31275 + 750,
    color = "grey20",
  ...
  annotate(
    ...
    x = 3.1, y = 22000,
    label = paste0(
      "Medicare reimburses drugs<br>at a fixed rate ",
      "(<span style='color:#e36402'>ASP</span> + 6%)"
    ),
    size = 4,
    color = "grey20",
    hjust = 0,
  ...
  annotate(
    ...
    x = 3.45, y = 12500,
    label = paste0(
      "The **darker** area shows the<br>provider's profit from a standard",
      "<br>(non-340B) drug transaction<sup>2</sup>"
    ),
    size = 4,
    color = "grey20",
    hjust = 0,
  ...
  annotate(
    ...
    x = 3.45, y = 7500,
    label = paste0(
      "The area between the <span style='color:#fd9a4e'>**orange**</span>",
      "<br> and <span style='color:#caa502'>**yellow**</span> lines represents",
      "<br> the additional profit from 340B"
    ),
    size = 4,
    color = "grey20",
    hjust = 0,
  ...
  annotate(
    ...
    x = 3.45, y = 2500,
    label = paste0(
      "In this case, each 200 mg dose<br>of Keytruda nets the provider",
      "<br>**$2,586** in additional 340B profit"
    ),
    size = 4,
    color = "grey20",
    hjust = 0,
  ...
  scale_fill_manual(
    values = c(
      "Drug cost" = "transparent",
      "Hospital list price" = "transparent",
      "Reimbursement price" = "transparent"
    )
  ) +
  labs(
    title = paste0(
      "340B lets providers save on drug acquisition costs"
    ),
    subtitle = paste0(
      "In this case, each 200 mg dose of Keytruda nets covered under 340B<br>",
      "nets Javon Bea **$2,586** in additional profit"
    ),
    caption = paste0(
      "test test"
    )
  ) +
  coord_cartesian(clip = "off", xlim = c(1, 4.1), ylim = c(0, 3e4)) +
  guides(fill = "none") +
  theme_minimal() +
  theme(
