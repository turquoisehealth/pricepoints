...
price_by_drg_df <- providers_df %>%
  ...
  group_by(billing_code, billing_code_label, severity) %>%
  summarise(
    ...
    xmin_pct = wtd.quantile(
      agg_rate_provider_med / medicare_rate, total_beds, 0.10
    ),
    lower_pct = wtd.quantile(
      agg_rate_provider_med / medicare_rate, total_beds, 0.25
    ),
    median_pct = wtd.quantile(
      agg_rate_provider_med / medicare_rate, total_beds, 0.50
    ),
    upper_pct = wtd.quantile(
      agg_rate_provider_med / medicare_rate, total_beds, 0.75
    ),
    xmax_pct = wtd.quantile(
      agg_rate_provider_med / medicare_rate, total_beds, 0.90
    ),
    drg_label = unique(paste0(
      billing_code_label, " ", severity
    )),
    count_label = paste0(
...
price_by_drg_df %>%
  ...
  geom_boxplot(
    aes(
      xmin = xmin_pct, xlower = lower_pct, xmiddle = median_pct,
      xupper = upper_pct, xmax = xmax_pct, y = NA
    ),
    width = 0.5,
    outliers = FALSE,
    stat = "identity",
  ...
  annotate(
    "rect",
    xmin = -Inf, xmax = Inf, ymin = 1.45, ymax = 1.8,
    fill = "grey80"
  ) +
  annotate(
    "rect",
    xmin = -Inf, xmax = 1.1, ymin = 1.45, ymax = 1.8,
    fill = "grey75"
  ) +
  annotate(
    "rect",
    xmin = 2.3, xmax = Inf, ymin = -Inf, ymax = 0.625,
    fill = "#e0ebea"
  ) +
  geom_richtext(
    aes(label = paste0("**", billing_code, "**")),
  ...
  coord_cartesian(xlim = c(0.75, 2.65)) +
  facet_grid(
    cols = vars(billing_code_label),
    rows = vars(severity),
    switch = "both"
  ...
  labs(
    title = paste0(
      "Hospitals charge more for deliveries with fewer complications,\n",
      "relative to Medicare benchmark pricing"
    ),
    x = paste0(
      "<span style='color:grey60;'>Percent of Medicare price</span>",
      "<br><span style='font-size:20px'>",
      "More complex delivery types ",
      "<span style='font-size:36px'>→</span></span>"
    ),
    ...
    caption = paste0(
      ...
      " providers<br>",
      "Boxes show the 10th, 25th, 50th, 75th, and 90th percentiles of ",
      "provider-level delivery prices compared to Medicare prices"
    )
  ) +
  theme(
    ...
    plot.margin = margin(r = 6, t = 6, b = 6, l = 6),
    axis.text.y = element_markdown(
      size = 14,
      margin = margin(r = 4),
      lineheight = 1.25,
...
providers_df %>%
  ...
  mutate(cbsa_label = fct_reorder(cbsa_label, median, .desc = TRUE)) %>%
  ggplot() +
  geom_vline(
  ...
  labs(
    ...
    caption = paste0(
      ...
      "Boxes show the 10th, 25th, 50th, 75th, and 90th percentiles of ",
      "provider-level delivery prices, weighted by bed count\n",
      "Markets are defined as the top 25 most populous 2023 Census ",
