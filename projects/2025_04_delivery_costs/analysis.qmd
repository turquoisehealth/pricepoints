...
rates_fil_df <- rates_df %>%
  ...
  filter(
    # These are actually mislabeled ASCs, psych, and cancer centers
    !provider_id %in% c(
  ...
  # Lots of "other" contract methodologies are actually medicare and medicaid
  filter(!coalesce(
...
provider_payer_df <- rates_fil_df %>%
  ...
  group_by(provider_id, billing_code, payer_id) %>%
  summarise(
    across(
...
states_hexbin_gdf %>%
  ...
  labs(
    title = paste0(
      "Alaska, Oregon, and New Jersey have the\n",
      "highest delivery prices nationwide"
    ),
    subtitle = paste0(
      "Compared to the national median price of ",
      us_code_price_df %>%
        filter(billing_code == "807") %>%
        pull(med_rate) %>%
        round() %>%
...
provider_payer_df %>%
  ...
  left_join(
    provider_cbsa_df %>%
      select(
        provider_id, billing_code,
        cbsa_label, billing_code_label
      ),
    by = c("provider_id", "billing_code")
  ) %>%
  ggplot() +
  ggridges::geom_density_ridges(
    aes(
      x = agg_rate_provider_payer_med,
      y = cbsa_label,
      group = cbsa_label,
      weight = total_beds
    ),
  ...
  facet_wrap(vars(payer_label), nrow = 1) +
  labs(
    title = paste0(
      "New York City is the most expensive large U.S. ",
      "market for childbirth"
  ...
  coord_cartesian(xlim = c(3000, 35000)) +
  theme(
    plot.title = element_text(
      size = 20,
      face = "bold",
