...
label_dollar_short <- function(x) {
  sapply(x, function(val) {
    if (is.na(val)) {
      return(NA_character_)  # Return NA as a character for consistency
    }
    ...
    if (abs_val >= 1e9) {
      paste0(sign_str, "$", round(abs_val / 1e9, 1), "B")
    } else if (abs_val >= 1e6) {
      paste0(sign_str, "$", round(abs_val / 1e6, 1), "M")
    } else if (abs_val >= 1e3) {
      paste0(sign_str, "$", round(abs_val / 1e3, 2), "K")
    } else {
...
code_desc_df <- tribble(
  ...
  "216", "Cardiac valve w MCC",
  "470", "Major join replacement no MCC",
...
payer_rates_df <- rates_df |>
  filter(payer_name == "Blue Shield of California") |>
  group_by(billing_code, billing_code_type, payer_name) |>
  summarize(med_rate_payer = median(canonical_rate), med_rate_all = first(med_rate_all)) |>
  left_join(code_desc_df, by = "billing_code") |>
  ungroup() |>
  mutate(
    code = factor(paste0("**", billing_code_type, " ", billing_code, "** - ", code_desc)),
    code = fct_reorder(code, med_rate_all)
  )

provider_rates_df <- rates_df |>
  group_by(billing_code, billing_code_type, provider_id, provider_name, health_system_name) |>
  summarize(med_rate_provider = median(canonical_rate), med_rate_all = first(med_rate_all)) |>
  left_join(code_desc_df, by = "billing_code") |>
  ungroup() |>
  mutate(
    code = factor(paste0("**", billing_code_type, " ", billing_code, "** - ", code_desc)),
...
ggplot() +
  geom_rug(
    data = payer_rates_df,
    aes(xintercept = med_rate_payer, fill = payer_name, color = payer_name),
    linewidth = 1.5,
    alpha = 0.7,
    length = unit(0.5, "cm"),
    sides = "b"
  ...
  geom_vline(
    data = payer_rates_df,
    aes(xintercept = med_rate_all),
    color = "grey40",
    linewidth = 1.5,
    alpha = 0.7
  ) +
  geom_dotplot(
    aes(x = med_rate_provider, fill = rate_label, color = rate_label),
    method = "histodot",
    stackgroups = TRUE,
    dotsize = 0.9
  ) +
  ...
  scale_x_continuous(
    name = "Median negotiated price",
    labels = label_dollar_short,
    n.breaks = 7
  ) +
  ...
  scale_fill_manual(
    name = "",
    values = c(
      "Blue Shield of California" = "#3786d0",
      "Hoag" = "#e59c35",
      "Other" = "grey75"
    )
  ) +
  ...
  facet_wrap(vars(code), ncol = 2, scales = "free") +
  labs(
    title = paste0(
      "<span style='color:#e59c35'>Hoag</span> rates in Orange County, CA"
    ),
    y = "**<span style='font-size:42px'>←</span> Higher priced procedures**",
  ) +
  theme(
    ...
    axis.title.y = element_markdown(size = 20, color = "grey60"),
...
rates_df2 <- rates_df |>
  ...
  mutate(
    rate_label = case_when(
      payer_name == "Blue Shield of California" ~ "Blue Shield",
      str_detect(tolower(health_system_name), "hoag") ~ "Hoag",
      .default = "Other"
    ),
    rate_label = factor(rate_label, levels = c(
      "Blue Shield",
      "Hoag",
      "Other"
    )),
    code = factor(paste0("**", billing_code_type, " ", billing_code, "** - ", code_desc)),
    code = fct_reorder(code, med_rate),
  )

rates_df2 |>
ggplot() +
  ggridges::geom_density_ridges(
    aes(x = canonical_rate, fill = rate_label, color = rate_label, y = rate_label),
    alpha = 0.5,
    quantile_lines = TRUE,
    quantiles = 2,
  ...
  geom_text(
    data = rates_df2 |>
      summarize(count = paste0("(", n(), ") "), .by = c("code", "rate_label")),
    aes(x = Inf, fill = rate_label, color = rate_label, y = rate_label, label = count),
    nudge_y = 0.25,
    size = 5,
    hjust = 1
  ) +
  ...
  scale_y_discrete(expand = expansion(add = c(0.01, 0.1))) +
  scale_fill_manual(
    name = "",
    values = c(
      "Blue Shield" = "#3786d0",
  ...
  labs(
    title = paste0(
      "<span style='color:#3786d0'>**Blue Shield**</span> vs.",
      " <span style='color:#e59c35'>**Hoag**</span> negotiated rates in the Los ",
      "Angeles area"
    ),
    subtitle = "Compared to all other rates in the Los Angeles-Long Beach-Anaheim CBSA (<span style='color:grey50'>**shown in grey**</span>)<br>Numbers in parenthesis show the count of rates in each group",
    y = "**<span style='font-size:42px'>←</span> Higher priced procedures**",
  ) +
  facet_wrap(vars(code), ncol = 2, scales = "free") +
  theme(
    ...
    axis.text.x = element_text(size = 10),
    axis.ticks.y = element_blank(),
    axis.title.y = element_markdown(size = 20, color = "grey20", margin = margin(r = 8)),
    axis.text.y = element_blank(),
    panel.background = element_rect(fill = "#e0ebea"),
