...
provider_comp_df |>
  ...
  labs(
    title = paste0(
      ...
      " <span style='color:#4186d0'>Blue Shield</span> than other hospitals",
      " in Orange County"
    ),
    subtitle = paste0(
...
```{r payer_comp_plot, message=FALSE}
#| column: page
#| out-width: 100%
#| fig-width: 11
#| fig-height: 11
...
payer_comp_df <- rates_df |>
  ...
  mutate(
    rank = factor(rank),
    payer_market_share = scales::label_percent(accuracy = 0.1, trim = FALSE)(payer_market_share),
    ...
    # United's ranking here is due to missing some values, so bump Anthem to
    # where it belongs
    payer_name = fct_relevel(
      payer_name,
      "**Anthem**",
      after = Inf
    )
...
payer_comp_df |>
  ...
  geom_rect(
    data = tribble(
      ~"proc_type", ~"xmin", ~"xmax",
      "Inpatient procedures", 2.5, 3.5,
      "Outpatient procedures", 2.5, 3.5
    ),
    aes(
      xmin = xmin,
      xmax = xmax,
  ...
  labs(
    title = paste0(
      "<span style='color:#4186d0'>Blue Shield</span> rates at ",
      "<span style='color:#e59c35'>Hoag</span> are ",
      "average among large insurers in Orange County"
    ),
    subtitle = paste0(
      "Across a basket of common procedures, Blue Shield",
      " prices are consistently in the middle of the market"
    ),
    y = "",
    x = "",
    caption = paste0(
      "Market share percentage is for Orange County, ",
      "data is from Policy Reporter<br>",
      "Based on ", nrow(payer_comp_df),
      " rates negotitated between Hoag and major insurers in Orange County"
    )
  ) +
  guides(
    fill = guide_legend(
...
```{r medicare_comp_map, message=FALSE, warning=FALSE}
#| column: page
#| out-width: 100%
#| fig-width: 8
#| fig-height: 9
...
medicare_comp_df <- rates_df |>
  ...
  mutate(
    prov_fct = case_when(
      ...
      str_detect(provider_name, "USC") ~ "USC",
      .default = "Other"
    ),
    prov_fct = fct_rev(factor(
      prov_fct,
      levels = c("Hoag", "UCI", "USC", "Other")
    ))
  ) |>
  st_as_sf(
...
medicare_comp_hist <- medicare_comp_df |>
ggplot() +
  geom_dotplot(
  ...
  scale_x_continuous(
    name = paste0(
      "**Provider median negotiated rate**",
...
ggplot() +
  ...
  geom_sf(aes(color = prov_fct), size = 4, alpha = 0.8) +
  scale_color_manual(values = provider_colors) +
  annotate(
    "text",
    y = 33.675, x = -118.475,
  ...
  labs(
    title = paste0(
      ...
      "the most expensive hospitals in the L.A. area"
    ),
    subtitle = paste0(
      "Across major insurers, Hoag prices are consistently ",
      "high relative to Medicare"
    ),
    caption = paste0(
      "Rates are benchmarked to Medicare then aggregated by hospital ",
      "across major payers<br>",
      "Not all hospitals are shown. Some are missing data or ",
      "hard to compare (e.g. Kaiser)<br>",
      "Based on ", medicare_comp_df |>
        pull(num_rates) |>
        sum() |>
        scales::label_comma()(),
