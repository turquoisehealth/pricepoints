...
hospital_info_df |>
  mutate(
    acquiring_nonprofit_short = ifelse(
      acquiring_nonprofit_short %in% c(
  ...
  geom_point(
    aes(
      x = acquired_date, y = hospital_name_short,
      shape = "Acquired", color = "Acquired",
      fill = "Acquired"
    ),
    size = 2
  ) +
  geom_point(
    aes(
      x = hospital_340b_date_start, y = hospital_name_short,
      shape = "Joined 340B", color = "Joined 340B", fill = "Joined 340B"
    ),
    size = 3.5
  ) +
  geom_point(
    aes(
      x = closed_date, y = hospital_name_short,
      shape = "Closed", color = "Closed", fill = "Closed"
    ),
    size = 3.5
  ) +
  labs(
...
cost_reports_df |>
  left_join(
    hospital_info_df |>
      ...
      select(
        ccn, acquiring_nonprofit_short, acquired_date, closed_date,
        hospital_340b_date_start, hospital_340b_date_end, hospital_name
      ),
    by = "ccn"
  ) |>
  mutate(
    time_from_acquisition = time_length(mcr_fy_end_date - acquired_date, "months"),
    hospital_340b_date_end = replace_na(hospital_340b_date_end, make_date(2028)),
    is_340B = between(mcr_fy_end_date, hospital_340b_date_start, hospital_340b_date_end),
