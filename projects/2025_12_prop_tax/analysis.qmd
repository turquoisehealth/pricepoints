...
hospital_info_df <- read_parquet("data/output/hospital_info.parquet") |>
  ...
  mutate(
    acquiring_nonprofit_short = case_match(
      ...
      "Tower Health" ~ "Community Health Systems → Tower Health",
    ...
    hospital_name_short = paste0(
      hospital_name_short,
      "<span style='font-family:mono'> (", hospital_state, ") </span>"
    )
  )
...
```{r status_timeline, message=FALSE, warning=FALSE}
#| column: page
#| out-width: 100%
#| fig-width: 8
#| fig-height: 9
...
status_timeline_df <- hospital_info_df |>
  mutate(
    acquiring_nonprofit_short = ifelse(
      acquiring_nonprofit_short %in%
        c(
          ...
          "CTCA → City of Hope",
          "HCA → Singing River Health System",
          "Tenet Healthcare → Wellstar Health System",
          "Westridge Capital → Cedars-Sinai",
          "Tenet Healthcare → Loyola Medicine"
        ),
      paste0(
...
status_timeline_340b_df <- status_timeline_df |>
  select(
    hospital_name_short,
    acquiring_nonprofit_short,
    ccn,
  ...
  left_join(
    cost_reports_df |>
      select(ccn, mcr_fy_end_date, mcr_dsh_pct),
    by = "ccn"
  ) |>
  mutate(hospital_340b_type = str_sub(hospital_340b_id, 1, 3)) |>
  filter(
    (mcr_dsh_pct >= 0.1175 & hospital_340b_type == "DSH") |
      (mcr_dsh_pct >= 0.08 & hospital_340b_type == "RRC") |
      hospital_340b_type == "CAH"
  ) |>
  summarize(
    min_date = min(pmax(mcr_fy_end_date, acquired_date, na.rm = TRUE)),
    max_date = max(pmin(mcr_fy_end_date, closed_date, na.rm = TRUE)),
    .by = c(
      "ccn",
      "hospital_name_short",
      "hospital_340b_type",
  ...
  mutate(
    max_date = as_date(ifelse(
      max_date >= make_date(2024, 9),
      make_date(2026, 11),
      max_date
    ))
  )
...
status_timeline_df |>
  ...
  geom_point(
    aes(
      x = acquired_date,
      y = hospital_name_short,
      shape = "Hospital acquired",
      color = "Hospital acquired",
      fill = "Hospital acquired"
    ),
    size = 2
  ) +
  geom_point(
  ...
  geom_point(
    aes(
      x = closed_date,
      y = hospital_name_short,
      shape = "Hospital closed",
      color = "Hospital closed",
      fill = "Hospital closed"
    ),
    size = 3.5
  ) +
  labs(
    title = "For-profit hospitals acquired by nonprofits quickly join 340B",
    x = "",
    y = "",
    caption = paste0(
      "Data sourced from hospital acquisition news coverage; ",
      "340B enrollment data from the HRSA 340B OPAIS database"
    )
  ) +
  scale_x_datetime(expand = expansion(mult = c(0.05, 0.01))) +
  scale_color_manual(
    name = "",
    values = c(
      "Hospital acquired" = status_timeline_color,
      "340B eligible" = "palegreen3",
      "Joined 340B" = status_timeline_color,
      "Hospital closed" = "indianred"
    ),
    breaks = c(
      "Hospital acquired",
      "340B eligible",
  ...
  coord_cartesian(
    clip = "off",
    xlim = as.POSIXct(c(make_date(2016), make_date(2027)))
  ) +
  theme_minimal() +
  theme(
    strip.clip = "off",
  ...
  theme(
    ...
    axis.title.y.left = element_text(margin = margin(l = -100)),
    axis.text.y.left = element_markdown(),
    panel.grid.minor.x = element_blank(),
    legend.position = "top",
    legend.text = element_text(size = 12),
    legend.margin = margin(r = 12, t = 8, b = 8),
    legend.box.margin = margin(t = 6, b = 12, l = -236),
...
macneal_tax_bills_df |>
  ...
  labs(
    title = "MacNeal's property tax bill dropped by ~$2.5M after acquisition",
    subtitle = paste0(
...
cost_reports_df |>
  ...
  mutate(
    ...
    closed = ifelse(closed, "Hospital closed", NA)
  ) |>
  ...
  geom_vline(
    aes(
      xintercept = 0,
      color = "Acquisition date",
      shape = "Acquisition date",
      linetype = "Acquisition date"
    )
  ) +
  geom_line(
    aes(
  ...
  scale_linetype_manual(
    ...
    values = c(
      ...
      "Part of 340B" = "solid",
      "Hospital closed" = "solid"
    ),
    breaks = c("Acquisition date", "Part of 340B", "Hospital closed"),
    na.value = "solid"
  ) +
  scale_color_manual(
    name = "",
    values = c(
      "Acquisition date" = "grey10",
      "Part of 340B" = "dodgerblue1",
      "Hospital closed" = "indianred"
    ),
    breaks = c("Acquisition date", "Part of 340B", "Hospital closed")
  ) +
  ...
  theme(
    strip.clip = "off",
    strip.placement = "outside",
    strip.text.x.top = element_markdown(face = "bold", size = 10.7)
  ) +
  theme(
    plot.title = element_markdown(
      size = 18.5,
    ...
    legend.margin = margin(r = 12, t = 8, b = 8),
    legend.box.margin = margin(t = 6, l = -60),
    legend.background = element_rect(color = "black"),
    legend.key.spacing.x = unit(0.7, "cm")
  )
```
